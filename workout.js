document.addEventListener('DOMContentLoaded', () => {
    ...
    // ELEMEN BARU UNTUK FILTER DI MODAL
    ...
    // ===================================================================
    // == ANALYTICS DASHBOARD & PROGRESS CHART FUNCTIONS ==
    // ===================================================================
    
    function renderAnalyticsDashboard() {
        updateRecapUI();
        const range = getRecapDateRange(currentDate, currentRecapFilter);
        const analyticsData = calculateAnalytics(range.start, range.end);
        
        if (currentRecapFilter === 'day') {
            recapDynamicCardTitleEl.textContent = 'Total Reps';
            animateCountUp(recapDynamicCardValueEl, analyticsData.totalReps);
        } else {
            const title = currentRecapFilter.charAt(0).toUpperCase() + currentRecapFilter.slice(1);
            recapDynamicCardTitleEl.textContent = `This ${title}'s Sessions`;
            animateCountUp(recapDynamicCardValueEl, analyticsData.workoutSessions);
        }

        animateCountUp(recapTotalMovesEl, analyticsData.totalMoves);
        animateCountUp(recapTotalSetsEl, analyticsData.totalSets);
        animateCountUp(recapTotalPRsEl, analyticsData.personalRecords.length);

        renderPRList(analyticsData.personalRecords);
        renderDistributionChart(analyticsData.setsByMuscleGroup);
    }

    function calculateAnalytics(startDate, endDate) {
        const allWorkouts = getWorkouts();
        const analytics = { totalSets: 0, totalReps: 0, workoutSessions: 0, totalMoves: 0, personalRecords: [], setsByMuscleGroup: {}, };
        ...
        for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
            ...
            if (dailyWorkouts.length > 0) { workoutDays.add(dateKey); }
            dailyWorkouts.forEach(exercise => {
                moves.add(exercise.name);
                if (exercise.categories.includes('Cardio')) return;
                let exerciseTopLift = topLifts[exercise.name] || { weight: 0, reps: 0 };
                exercise.sets.forEach(set => {
                    const weight = parseFloat(set.weight) || 0;
                    const reps = parseInt(set.reps) || 0;
                    if (weight > 0 && reps > 0) {
                        analytics.totalSets++;
                        analytics.totalReps += reps;
                        exercise.categories.forEach(cat => { if (cat !== 'Cardio') { analytics.setsByMuscleGroup[cat] = (analytics.setsByMuscleGroup[cat] || 0) + 1; } });
                        if (weight >= exerciseTopLift.weight) { exerciseTopLift = { weight, reps }; }
                    }
                });
                topLifts[exercise.name] = exerciseTopLift;
            });
        }
        analytics.workoutSessions = workoutDays.size;
        analytics.totalMoves = moves.size;
        analytics.personalRecords = Object.entries(topLifts).filter(([, lift]) => lift.weight > 0).map(([name, lift]) => ({ name, ...lift }));
        return analytics;
    }

    function renderPRList(prs) {
        recapPRListEl.innerHTML = '';
        if (prs.length === 0) { recapPRListEl.innerHTML = `<p class="text-center text-gray-400 py-4">No new PRs in this period.</p>`; return; }
        prs.sort((a,b) => b.weight - a.weight).forEach(pr => {
            const item = document.createElement('div');
            item.className = 'flex justify-between items-center p-2 -m-2 rounded-lg cursor-pointer hover:bg-gray-100 pr-item-clickable';
            item.dataset.exerciseName = pr.name;
            item.innerHTML = `<span class="font-semibold text-gray-700">${pr.name}</span><span class="font-bold text-pink-500">${pr.weight} kg <span class="font-medium text-gray-400">× ${pr.reps}</span></span>`;
            recapPRListEl.appendChild(item);
        });
    }

    function renderDistributionChart(setsData) {
        if (distributionChartInstance) { distributionChartInstance.destroy(); }
        const sortedData = Object.entries(setsData).sort(([, a], [, b]) => b - a);
        const labels = sortedData.map(item => item[0]);
        const data = sortedData.map(item => item[1]);
        const totalSets = data.reduce((sum, value) => sum + value, 0);
        const chartColors = ['#ec4899', '#d946ef', '#a855f7', '#8b5cf6', '#60a5fa', '#34d399', '#f472b6'];

        chartCenterTextEl.innerHTML = '';
        recapCustomLegendEl.innerHTML = '';

        if(data.length === 0) {
             recapDistributionChartCanvas.style.display = 'none';
             recapPRListEl.innerHTML = `<p class="text-center text-gray-400 py-4">No strength training logged.</p>`;
             return;
        }
        recapDistributionChartCanvas.style.display = 'block';

        chartCenterTextEl.innerHTML = `<span class="text-3xl font-bold text-gray-800">${totalSets}</span><span class="text-sm font-medium text-gray-500">Total Sets</span><span class="text-xs text-gray-400 mt-1">${labels.length} groups</span>`;
        
        labels.forEach((label, index) => {
            const color = chartColors[index % chartColors.length];
            const value = data[index];
            const legendItemHTML = `<div class="flex items-center text-sm"><span class="w-3 h-3 rounded-sm mr-2" style="background-color: ${color};"></span><span class="font-medium text-gray-600">${label}</span><span class="ml-auto pl-2 font-semibold text-gray-800">${value}</span></div>`;
            recapCustomLegendEl.innerHTML += legendItemHTML;
        });

        distributionChartInstance = new Chart(recapDistributionChartCanvas, {
            type: 'doughnut',
            data: {
                labels: labels,
                datasets: [{
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: { callbacks: { label: function(context) { const label = context.label || ''; const value = context.raw || 0; return ` ${label}: ${value} sets`; } } },
    ...
    function updateRecapUI() {
        ...
        if (currentRecapFilter === 'day') { recapPeriodDisplay.textContent = range.start.toLocaleDateString('en-US', { ...formatOptions, year: 'numeric' }); } 
        else if (currentRecapFilter === 'month') { recapPeriodDisplay.textContent = range.start.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }); }
        else { recapPeriodDisplay.textContent = `${range.start.toLocaleDateString('en-US', formatOptions)} - ${range.end.toLocaleDateString('en-US', formatOptions)}`; }
        document.querySelectorAll('.recap-filter-btn').forEach(btn => btn.classList.remove('active'));
        document.getElementById(`recap-filter-${currentRecapFilter}`).classList.add('active');
    }
    ...
    function getExerciseHistory(exerciseName) {
        ...
        for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
            const dateKey = toDateKey(d);
            const dailyWorkouts = allWorkouts[dateKey] || [];
            const relevantExercises = dailyWorkouts.filter(ex => ex.name === exerciseName);
            if (relevantExercises.length > 0) {
                let maxWeight = 0;
                relevantExercises.forEach(ex => { ex.sets.forEach(set => { const weight = parseFloat(set.weight) || 0; if (weight > maxWeight) { maxWeight = weight; } }); });
                if (maxWeight > 0) { history.push({ date: new Date(d), maxWeight: maxWeight }); }
            }
        }
        return history;
    }

    function renderProgressChart(exerciseName, historyData) {
        if (progressChartInstance) { progressChartInstance.destroy(); }
        progressChartTitle.textContent = `${exerciseName} Progress`;
        const labels = historyData.map(item => item.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }));
        const data = historyData.map(item => item.maxWeight);
        progressChartInstance = new Chart(exerciseProgressChartCanvas, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
    ...
    function getRecapDateRange(date, filter) {
        const d = new Date(date);
        if (filter === 'day') { return { start: d, end: d }; }
        if (filter === 'month') { const start = new Date(d.getFullYear(), d.getMonth(), 1); const end = new Date(d.getFullYear(), d.getMonth() + 1, 0); return { start, end }; }
        const day = d.getDay(); const diff = d.getDate() - day + (day === 0 ? -6 : 1);
        const start = new Date(d.setDate(diff)); const end = new Date(new Date(start).setDate(start.getDate() + 6));
        return { start, end };
    }
    ...
    searchInput.addEventListener('input', () => { const activeCategoryBtn = document.querySelector('.category-btn.bg-pink-500'); const category = activeCategoryBtn ? activeCategoryBtn.textContent : 'All'; renderExerciseList(category, searchInput.value); });
    showAddCustomFormBtn.addEventListener('click', () => { mainExerciseSelectionView.classList.add('hidden'); addCustomExerciseView.classList.remove('hidden'); });
    showExerciseListViewBtn.addEventListener('click', () => { mainExerciseSelectionView.classList.remove('hidden'); addCustomExerciseView.classList.add('hidden'); });
